...
RSpec.describe 'Show orcid dashboard pages' do
  ...
  context 'when user is logged in but is not in the business case workgroup' do
    ...
    it 'shows the orcid dashboard stanford overview viz in tab' do
      ...
      expect(page).to have_content('ORCID Adoption Dashboards')
      within('#overview_tab') do
        expect(page).to have_css("#tableau-viz[src=\"#{Settings.dashboard.orcid_adoption.stanford_overview}\"]")
      end
    end

    it 'shows the orcid dashboard schools and departments viz in tab' do
      visit orcid_adoption_dashboard_path(tab: 'schools')
      within('#schools_tab') do
        expect(page).to have_css("#tableau-viz[src=\"#{Settings.dashboard.orcid_adoption.schools_and_departments}\"]")
      end
    end

    it 'does not show the orcid dashboard individual researchers viz in tab' do
      visit orcid_adoption_dashboard_path(tab: 'researchers')
      expect(page).to have_content('This dashboard is only available to select users.')
      within('#researchers_tab') do
  ...
  context 'when user is logged in and is in the business case workgroup' do
    ...
    it 'shows the orcid dashboard individual researchers viz in tab' do
      visit orcid_adoption_dashboard_path(tab: 'researchers')
      within('#researchers_tab') do
        expect(page).to have_css("#tableau-viz[src=\"#{Settings.dashboard.orcid_adoption.individual_researchers}\"]")
      end
    end
  end
